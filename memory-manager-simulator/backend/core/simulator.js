import MemoryProcess from '../models/process.js';
import MemoryPartition from '../models/partition.js';
...
class MemorySimulator {
  ...
  compactMemory() {
    // Con segmentos variables, las particiones dinámicas evitan fragmentación interna;
    // en bloques fijos, parte de la memoria quedaría desperdiciada por diseño.
    const residentProcesses = this.getResidentProcesses().sort(sortByBaseAddress);
    let cursor = this.config.osBytes;
  ...
  captureCurrentMemorySnapshot() {
    // Ensure this tick's snapshot exists
  ...
  getState() {
    // Ensure we have at least the current snapshot
